...
while (not close):  # As long as there are more pizzas to order
    ...
    while (status == "Alter"):  # As long as the status is "Alter"
        ...
        while (sizeValid == False):  # Validation loop
            size = raw_input("The size you have entered is invalid. Please re-enter the size from one of the options above: ")  # Print error message and ask for correction
        ...
        base = raw_input("\nPlease enter the pizza base you would like: ")  # Input prompt
        
        baseValid = False  # Set flag as invalid
        
        # Check if the base is valid
        for count in range(2):  # Iterate 2 times for 2 bases
        ...
        while (baseValid == False):  # Validation loop
            base = raw_input("The base you have entered is invalid. Please re-enter the base from one of the options above: ")  # Print error message and ask for correction
            
            # Check if the base is valid
            for count in range(2):  # Iterate 2 times for 2 sizes
                if base == basesAvailable[count]:  # If a match is found from the available bases
                    baseValid = True  # Set flag as valid
                    bases[count] = True  # Set flag as selected
        ...
        if (toppingChoice <= 3) and (toppingChoice >= 0):  # If the number of toppings is in the acceptable range
             toppingChoiceValid = True  # Set flag as valid
        
        while (toppingChoiceValid == False):  # Validation loop
            toppingChoice = int(input("You have entered an invalid number of toppings. Please re-enter any whole number 0 and 3: "))  # Throw error message and ask for correction
        ...
        for countO in range(toppingChoice):  # Iterate as many times as the toppings taken
            ...
            topping = raw_input("\nPlease enter topping " + str(countO + 1) + " of the pizza you would like: ")  # Input prompt
            ...
            while (toppingValid == False):  # Validation loop
                topping = raw_input("The topping you have entered is invalid. Please re-enter the topping from one of the options above: ")  # Print error message and ask for correction
        ...
        status = raw_input("\nDo you want to Alter your order, Confirm or Not proceed? ")  # Input prompt
    ...
    close = input("\nDo you want to exit the program? ")  # Input prompt
...
for count in range(6):  # Iterate 6 times for 6 toppings
    ...
    if totalToppings[count] > higest:  # If the current topping sold more than the running most popular topping
        higest = totalToppings[count]  # Update the running most popular topping
...
print toppingsAvailable[highestIndex], "was the most popular topping and accounted for", ((higest/toppingsSum) * 100.0), "% of the toppings sales."  # Output the most popular toppings
